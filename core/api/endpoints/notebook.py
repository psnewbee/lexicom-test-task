...
async def add_info(input_data: ContactInfoModel) -> Any:
    ...
    db_record_exist: bool = bool(await redis_service.exists(phone))
    await redis_service.hmset(name=phone, mapping=input_data)
    
    if db_record_exist:
        return Response(content="Address record updated", status_code=200)
    return Response(content="Address record created", status_code=201)
